    ...
    function draw() {
        if (life == 0) {
            ...
            if (localStorage.getItem("tocke") < tocke) {
                ...
                localStorage.setItem("time", izpisTimer);
                storage.innerHTML = "Tvoj najboljši rezultat je: " + localStorage.getItem("tocke") + " tock v času: " + localStorage.getItem("time");
            }
        }
        else {
            //zoga
            ...
            for (i = 0; i < bricks.length; i++) {
                for (j = 0; j < bricks[i].length - 2; j++) {
                    if (bricks[i][j] == 1 || bricks[i][j] == 2 || bricks[i][j] == 3 || bricks[i][j] == 4) {
                        ctx.beginPath();
